...
class MyWebConsumer(AsyncWebsocketConsumer):
    ...
    def get_message(self, user_from, message, room):
        return Messages.objects.filter(user_from=user_from, message=message, room=room)[
            len(
                Messages.objects.filter(user_from=user_from, message=message, room=room)
            )
            - 1
        ]
    ...
    def user_new_messages_true(self, user):
        return User.objects.filter(username=user).update(new_messages=True)

    @database_sync_to_async
    def user_new_messages_false(self, user):
        return User.objects.filter(id=user).update(new_messages=False)
    ...
    def user_connections_add(self, from_user, to_user, from_con, to_con):
        return from_user.connections.add(
            Requests.objects.get(user_from=from_con, user_to=to_con)
        ), to_user.connections.add(
            Requests.objects.get(user_from=from_con, user_to=to_con)
        )
            ...
            if (
                ...
                if type_notice == "request_notice":
                    ...
                    if text["type_post"] == "project":
                        ...
                        blog = await database_sync_to_async(Blog_list.objects.get)(
                            id=text["id"]
                        )
                    ...
                    for i in list:
                        if (
                            text in i.username
                            and i.username != self.scope["user"].username
                        ):
                ...
                if json.loads(text_data)["user_from"] != "":
                    ...
                    mes = await self.get_message(user_from, message, room)
                    await self.room_message_add(mes, room)

                    await self.user_new_messages_true(json.loads(text_data)["user_to"])
    ...
    async def return_search(self, event):
        await self.send(
            text_data=json.dumps(
                {"results": event["message"], "type": "results_search"}
            )
        )
    ...
    async def chat_message(self, event):
        ...
        list = await self.get_all_rooms(self.scope["user"])
        list2 = []
        for i in list:
            list2 += await self.get_all_messages(i)
        ...
        if not bool:
            ...
            print(2, bool)
            if "user_read" in event["message"]:
                print(event["message"]["my_id"], self.scope["user"].id)
                await self.user_new_messages_false(event["message"]["my_id"])
                await self.send(
                    text_data=json.dumps({"msg": event["message"], "type": "check_msg-check_viewed"})
                )
            else:
                await self.send(
                    text_data=json.dumps({"msg": event["message"], "type": "check_msg"})
    ...
    async def request_req(self, event):
        await self.send(
            text_data=json.dumps({"msg_data": event["msg_data"], "type": "request_req"})
        )
