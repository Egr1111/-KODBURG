...
class UserCreateForm(UserCreationForm):
    ...
    password2 = CharField(label="Повторите пароль", max_length=255, widget=TextInput(attrs={
        "class": "input-class col-xxl-3 col-lg-5 col-md-8 col-sm-12",
        "placeholder": "password",
        "type": "password"
    }))
    ...
    def clean(self):
        ...
        for i in username:
            if not i.isalpha() and i not in sim:
                self.add_error('username', 'Введите правильное имя пользователя. Оно может содержать только буквы, цифры и знаки @/+/-/_.')
                break
        return cleaned_data
    ...
    class Meta:
        ...
        widgets = {
            ...
            "email": TextInput(attrs={"class": "input-class col-12",
                                      "placeholder": "email",
                                      "type": "email"}),
            "first_name": TextInput(attrs={"class": "input-class col-12",
                                           "placeholder": "Как вас зовут?",
                                           }),
            "last_name": TextInput(attrs={"class": "input-class col-12",
