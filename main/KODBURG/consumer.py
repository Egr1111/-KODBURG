...
class MyWebConsumer(AsyncWebsocketConsumer):
    ...
    # @database_sync_to_async
    # def connection_del(self, from_user, to_user):
    #     return Requests.objects.filter(user_from = from_user, user_to = to_user).delete()
    ...
    async def connect(self):
        if self.scope["user"].is_authenticated:
            ...
            if self.group_name == f"TotalConsumer-{self.scope['user'].id}" and "." not in self.group_name:
                ...
                # if check_from != 0:
                #     if not database_sync_to_async(Requests.objects.get)(user_from = self.scope["user"], user_to = to_user).onl_from:
                #         await self.req_updateFrom(self.scope["user"], to_user)
                    
                # elif check_to != 0:
                #     if not database_sync_to_async(Requests.objects.get)(user_from = to_user, user_to = self.scope["user"]).onl_to:
                #         await self.req_updateTo(self.scope["user"], to_user)
                # else:
                #     await self.create_req(self.scope["user"], to_user)
                #     await self.user_connections_add(self.scope["user"], to_user)
                #     await self.channel_layer.group_send(
                #         self.group_name,
                #         ({
                #             "type": "request.req",
